...
public class PatternExam1 {
	public static void main(String[] args) {
		String[] dt = {"Friday","sleep","집","9to6","1004","single","Mom","JAVA",":)"};
		//Pattern p = Pattern.compile("[a-z]*");		//영문 소문자로만 구성된 데이터
		//Pattern p = Pattern.compile("[0-9a-z]*");		//숫자와 영문 소문자로만 구성된 데이터
		//Pattern p = Pattern.compile("[A-Za-z]*");		//영문 대문자 또는 영문 소문자로만 구성된 데이터
		//Pattern p = Pattern.compile("s[A-Za-z]*");	//s로 시작하면서 영문 대문자 또는 영문 소문자로만 구성된 데이터
		//Pattern p = Pattern.compile("[가-힣]*");		//한글로만 구성된 데이터
		//Pattern p = Pattern.compile("e$");			//특정 문자로 끝나는 경우 문자 뒤에 $
		//Pattern p = Pattern.compile("^s");			//특정 문자로 시작하는 경우 문자 앞에 ^
